...
from ImportData import Mitre_mapping
...
class Net:
    ...
    def checkAlert(self, alert):
        ...
        for tactic in alert.tactics:
            phases = Mitre_mapping[tactic]
            if phases == 0:
                return
            # Check if phases is int or list
            if type(phases) == int:
                phases = [phases]
            for phase in phases:
    ...
    def createEdges(self):
        for i in range(len(self.techniques)):
            for j in range(len(self.techniques)):
                if self.techniques[i].start < self.techniques[j].end and self.techniques[i].phase < self.techniques[j].phase:
                    self.edges.append((self.techniques[i], self.techniques[j]))
        
        transitive_reduction = self.edges.copy()
        for i, j in self.edges:
            for k in self.techniques:
                if (i, k) in self.edges and (k, j) in self.edges: 
                    transitive_reduction = [item for item in transitive_reduction if item != (i, j)]
                    break
